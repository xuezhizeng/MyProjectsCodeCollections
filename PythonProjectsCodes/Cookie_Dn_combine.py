...
try:
    os.mkdir('./Combined_cookie_htmls')
except:
    print 'exists'
...
#         print 'X',
        fail.append(name)
    with open('Combined_cookie_htmls/{}.html'.format(name), 'w+') as fl:
        fl.write(one.text)
#     print 'O',
...
if __name__=='__main__':
	...
	print 'fail exists'
	    ...
	    exit()
            ...
            print len(os.listdir('./Combined_cookie_htmls'))
            os.remove('fail.csv')
	    print 2
            exit()
	...
	print 'fail not exists'
        df=pd.read_csv('CombineAll_username.csv')
