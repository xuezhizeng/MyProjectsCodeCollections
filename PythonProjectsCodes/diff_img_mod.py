...
def batch(subber):
    ...
    #print 'images count',len(subber),'  used ',past,'s', now
    print 'used '+past+'s|',
...
if __name__=='__main__':
    ...
    # choose=raw_input('which set (1-80)?: ')
    csv='./csv/'
    try:
    ...
    df=pd.read_csv(csv+'img_'+str(choose)+'.csv')
    nn=df.shape[0]
    print nn,' pics'
    ...
    for i in spl(urls, many):
        ...
        batch(i)
        p+=many/float(nnn)
        print '{:.2f}% '.format(p*100)+str(many*q)+'/'+str(nnn),
        #print p*100,'%', many*q,'/', nnn,'|',
